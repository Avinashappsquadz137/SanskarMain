...
asset_zip_domain = env == "sandbox" ? "https://sandbox.assets.juspay.in" : "https://public.releases.juspay.in"
asset_zip_url = asset_zip_domain + '/hyper/bundles/in.juspay.merchants/' + merchant_name+ '/ios/release/assets.zip'

puts "[HyperSDK] Downloading assets from: " + asset_zip_url
...
if clean_assets || response.code == '200'
    ...
    FileUtils.rm_rf(Dir[ assets_path + "/payments-in.juspay.hyperpay-*.json" ])
end
...
if response.code == '200'
    ...
    puts "[HyperSDK] Download complete!"
    puts "[HyperSDK] Extracting assets..."
    system("unzip", "-q", "-o", "-j", local_zip_file, "-d", hyper_sdk_iphone_framework_path)
    if is_xcframework
        system("unzip", "-q", "-o", "-j", local_zip_file, "-d", hyper_sdk_simulator_framework_path)
    end
    ...
    puts "[HyperSDK] Extraction completed successfully!"

else
    puts "[HyperSDK] Error downloading zip file: #{response.code} - #{response.message}"
