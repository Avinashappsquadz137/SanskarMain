...
open class FSPagerViewCell: UICollectionViewCell {
    ...
    open var textLabel: UILabel? {
        ...
        let view = UIView(frame: CGRect(x: 0, y: 164, width: self.contentView.frame.size.width, height: self.contentView.frame.size.height - 164))
        view.isUserInteractionEnabled = false
        view.backgroundColor = UIColor.black.withAlphaComponent(0.6)
        
        let textLabel = UILabel(frame: CGRect(x: 0, y: 164, width: view.frame.size.width, height: view.frame.size.height))
    ...
    open var imageView: UIImageView? {
        ...
        let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: self.contentView.frame.size.width, height: 284))
    ...
    override open func layoutSubviews() {
        ...
        if let imageView = _imageView {
            imageView.frame = CGRect(x: 0, y: 0, width: self.contentView.frame.size.width, height: 284)
        }
        if let textLabel = _textLabel {
            textLabel.superview!.frame = {
                var rect = CGRect(x: 0, y: 164, width: self.contentView.frame.size.width, height:  self.contentView.frame.size.height-164)
                let height = textLabel.font.pointSize*1.5
                rect.size.height = self.contentView.frame.size.height-164
                rect.origin.y = 164//(self.contentView.frame.size.height-164)-height
                return rect
            }()
            textLabel.frame = {
