//
//  EPGColletionViewFlowLayout.swift
//  Sanskar
//
...
class EPGColletionViewFlowLayout: UICollectionViewFlowLayout {
    let cellWidth = 120.0
    let cellHeight = 80.0
    ...
    override func prepare() {
        ...
        if let sectionCount = collectionView?.numberOfSections, sectionCount > 0 {
            for section in 0...sectionCount - 1 {
                var channel: EpChannel?
                if section > 0 {
                    if let ch = self.epChannels?[section - 1] {
                        channel = ch
                    }
                }
                if let rowCount = collectionView?.numberOfItems(inSection: section), rowCount > 0 {
                    for item in 0...rowCount - 1 {
                        ...
                        if let _ = program {
                            ...
                            let width = (program!.duration / 60 ) * 120
