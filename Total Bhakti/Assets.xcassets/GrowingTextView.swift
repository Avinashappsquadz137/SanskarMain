...
open class GrowingTextView: UITextView {
    ...
    override open func layoutSubviews() {
        ...
        if (height_Constraint == nil) {
            height_Constraint = NSLayoutConstraint(item: self, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: height)
            addConstraint(height_Constraint!)
        }
        
        // Update height constraint if needed
        if height != height_Constraint!.constant {
            shouldScrollAfterHeightChanged = true
            height_Constraint!.constant = height
